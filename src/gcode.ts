...
function normalizePoints(paths: Point[][], svgScale: number): Point[][] {
  ...
  return paths.map((path) => path.map(({ x, y }) => ({ x: (x - minX) * scale, y: (y - minY) * scale })));
}
...
function svgPointsToGCodeCoords(paths: Point[][], pipeOD: number, axialOffset: number, rotationDeg: number): GCodeCoord[][] {
  ...
  return paths.map((path) => {
    ...
    return path.map((pt) => {
      ...
      prevAngle = angle;
      return { x: pt.y + axialOffset, y: Math.cos(angle), z: Math.sin(angle) };
    });
  });
}
...
/**
 * Generates a helical geodesic path on the pipe OD surface between two points.
 * Interpolates axial position and angle simultaneously — a straight line in
 * (x, θ) cylindrical space — always taking the shortest angular route.
 */
function generateGeodesicTransition(from: GCodeCoord, to: GCodeCoord, steps: number = 32): GCodeCoord[] {
  ...
  const result: GCodeCoord[] = [];
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    result.push({
      x: from.x + (to.x - from.x) * t,
      y: Math.cos(fromAngle + dAngle * t),
      z: Math.sin(fromAngle + dAngle * t),
    });
  }
  return result;
}

export function generateGCode(svg: Document, xStart: number, svgScale: number, rotation: number, pipeOD: number, svgRotation: number = 0, samplingStepSize: number = 0.5, transitionSteps: number = 32): { gcode: string; gcodeCoords: GCodeCoord[][]; gcodeTransitions: GCodeCoord[][] } {
  ...
  const gcodeTransitions: GCodeCoord[][] = gcodeCoords.slice(1).map((cut, i) => generateGeodesicTransition(gcodeCoords[i]![gcodeCoords[i]!.length - 1]!, cut[0]!, transitionSteps));
  ...
  const gcode = [
    ...
    ...gcodeCoords.flatMap((cut, i) => {
      const transition = gcodeTransitions[i - 1];
      return [...(transition ? ["TORCH_OFF", "", ...transition.map(coordToStr), ""] : []), "TORCH_ON", "", ...cut.map(coordToStr), ""];
    }),
    "TORCH_OFF",
    "",
  ].join("\n");

  return { gcode, gcodeCoords, gcodeTransitions };
}
